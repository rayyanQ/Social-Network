...
if (isset($_GET['u'])){
	...
	{
	echo "<meta http-equiv=\"refresh\" content=\"0; url=http://www.hubuddies.com/profile.php\">";	
	}
}
else{
	$username = $user;
...
<html>
		...
		<script type="text/javascript">
			$(document).ready(function(){
				$("#postbox").on("submit", function(event){
					...
					var user_to = '<?php echo $username; ?>';
					if (post.length > 0) {
						$.post('./ajax/profile_post.php',{post:post,privacy:privacy,user_to:user_to},function(data){
							$("#post_txt").val("");
							$("#postbox").after(data);
						});
					}
	...
	<body>
		...
		<center id="body">
			...
			<?php
			$FriendsQuery = mysqli_query($con, "SELECT friend_array FROM users WHERE username='$username'");
	        ...
	        $getposts = mysqli_query($con, "SELECT * FROM posts WHERE added_by='$username' AND deleted='no' OR user_posted_to='$username' AND deleted='no' ORDER BY id DESC LIMIT 30"); 
